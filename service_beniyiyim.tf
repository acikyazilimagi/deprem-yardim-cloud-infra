...
resource "aws_service_discovery_service" "beniyiyim-service" {
...
resource "aws_ecs_service" "beniyiyim-service" {
  ...
  service_registries {
    registry_arn   = aws_service_discovery_service.beniyiyim-service.arn
    container_name = "container-name"
    container_port = 80
  }
...
resource "aws_wafv2_web_acl_association" "beniyiyim-alb" {
  resource_arn = aws_lb.beniyiyim-alb.arn

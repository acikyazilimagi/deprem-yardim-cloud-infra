...
resource "aws_ecs_task_definition" "address-depremio-TD" {
  family                   = "address-depremio-TD"
  requires_compatibilities = ["FARGATE"]
  network_mode             = "awsvpc"
  cpu                      = 512
  memory                   = 1024
  ...
  container_definitions = jsonencode([
    {
      ...
      logConfiguration = {
        ...
        options = {
          awslogs-create-group  = "true"
          awslogs-group         = "/ecs/address-depremio"
          awslogs-region        = var.region
          awslogs-stream-prefix = "ecs"
        }
      }
...
resource "aws_ecs_service" "address-depremio-service" {
  name            = "address-depremio-service"
  cluster         = aws_ecs_cluster.base-cluster.id
  task_definition = aws_ecs_task_definition.address-depremio-TD.id
  desired_count   = 1
  depends_on = [
    aws_ecs_cluster.base-cluster,
    aws_ecs_task_definition.address-depremio-TD,
  ]
  launch_type = "FARGATE"
